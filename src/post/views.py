...
from ..profile.service import is_blocked
...
async def get_posts_from_hashtag(hashtag: str, token: str = None, db: Session = Depends(get_db)):
    user_id = None
    if token:
        user = await get_current_user(db, token)
        if user:
            user_id = user.id
    return await get_posts_from_hashtag_svc(db, hashtag, user_id)


@router.get("/dashboard")
async def get_random_posts(
    page: int = 1, limit: int = 5, hashtag: str = None, token: str = None, db: Session = Depends(get_db)
):
    user_id = None
    if token:
        user = await get_current_user(db, token)
        if user:
            user_id = user.id
    return await get_random_posts_svc(db, page, limit, hashtag, user_id)
...
async def get_post(post_id: int, token: str = None, db: Session = Depends(get_db)):
    ...
    if token:
        user = await get_current_user(db, token)
        if user:
            if await is_blocked(db, user.id, db_post.author_id):
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="invalid post id"
                )
