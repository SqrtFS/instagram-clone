...
@router.post("/block/{username}", status_code=status.HTTP_204_NO_CONTENT)
async def block(username: str, token: str, db: Session = Depends(get_db)):
    db_user = await get_current_user(db, token)
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="invalid token"
        )
    res = await block_svc(db, db_user.username, username)
    if res == False:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="could not block"
        )


@router.post("/unblock/{username}", status_code=status.HTTP_204_NO_CONTENT)
async def unblock(username: str, token: str, db: Session = Depends(get_db)):
    db_user = await get_current_user(db, token)
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="invalid token"
        )
    await unblock_svc(db, db_user.username, username)